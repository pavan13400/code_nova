...
import * as FileSystem from 'expo-file-system/legacy';
import * as Sharing from 'expo-sharing';
import { Activity, HeartPulse, Plus, Share2, Upload, X } from 'lucide-react-native';
  // ADDED ImagePicker for the gallery OCR
...
import { useRouter } from "expo-router";
  ...
  export default function MedicalQRScreen({ onGoToScanner }) {
    ...
    // ==========================================
    const [formData, setFormData] = useState({
      name: '', age: '', blood: '', phone: '', // <-- Added phone
      insurance: null, // <-- Added insurance to hold OCR data
      allergies: [], meds: [], surgeries: [], contacts: []
    });

    const [tempInputs, setTempInputs] = useState({ allergy: '', med: '', surgery: '', contact: '' });
    const [qrString, setQrString] = useState(null);
    const [isScanningOCR, setIsScanningOCR] = useState(false); // <-- Added loading state for OCR
    const qrRef = useRef();
    ...
    // --- FAKE BACKEND OCR LOGIC (GALLERY) ---
  // --- REAL BACKEND OCR LOGIC ---
    const handleInsuranceUpload = async () => {
      
      const permissionResult = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (!permissionResult.granted) {
        Alert.alert("Permission Required", "We need photo gallery access to upload your insurance card.");
        return;
      }

      const result = await ImagePicker.launchImageLibraryAsync({
        mediaTypes: ['images'],
        quality: 0.5, // Keep quality low to speed up the network request
        base64: true, // <-- CRUCIAL: We need the image converted to text data
      });
      ...
      if (!result.canceled && result.assets[0].base64) {
        ...
        try {
          // 1. Format the image for the Free OCR API
          ...
          const response = await fetch('https://api.ocr.space/parse/image', {
            method: 'POST',
            headers: {
              // 'helloworld' is a public testing key. If it gets rate-limited, 
              // you can get your own free one at ocr.space in 2 seconds.
              'apikey': 'helloworld', 
            },
            body: formData,
          });
          ...
          // 4. Get the massive block of text the AI found
          const extractedText = data.ParsedResults[0]?.ParsedText || "";
          if (!extractedText) throw new Error("No text found on image");

          console.log("AI Found this text:", extractedText); // Check your terminal to see what it read!

          // 5. Use Regex to hunt for the specific numbers in the messy text
          const policyMatch = extractedText.match(/(?:Policy|ID|Member|No)[\s#:-]*([A-Z0-9-]+)/i);
          const groupMatch = extractedText.match(/(?:Group|Grp)[\s#:-]*([A-Z0-9-]+)/i);
          
          // Split the text into clean lines, removing empty spaces
          const lines = extractedText.split('\n').map(line => line.trim()).filter(line => line.length > 0);

          // Hunt for a line that actually sounds like an insurance provider
          let providerName = lines.find(line => /(Insurance|Health|Life|Medical|Cross|Shield|Star)/i.test(line));

          // Fallback: If it didn't find those words, skip useless headers like "PAN NO."
          if (!providerName) {
            providerName = lines.length > 1 && lines[0].toUpperCase().includes("PAN") ? lines[1] : lines[0];
          }

          // Clean up any weird trailing commas the OCR might have caught
          providerName = providerName ? providerName.replace(/[,;:-]$/, '').trim() : "Unknown Provider";

          const realExtractedData = {
            provider: providerName,
            policyNumber: policyMatch ? policyMatch[1] : "Not Found (Edit Manually)",
            groupNumber: groupMatch ? groupMatch[1] : "Not Found (Edit Manually)"
          };
          
          // 6. Save it to our state!
          setFormData(prev => ({ ...prev, insurance: realExtractedData }));
          Alert.alert("Upload Complete", "Insurance details extracted successfully!");

        } catch (error) {
          console.log("OCR Error: ", error);
          Alert.alert("Scan Failed", "Could not read the insurance card clearly. Try a clearer photo.");
        } finally {
    ...
    const shareQR = async () => {
      ...
      qrRef.current.toDataURL(async (data) => {
        try {
          ...
          if (await Sharing.isAvailableAsync()) {
            await Sharing.shareAsync(fileUri, { mimeType: 'image/png', dialogTitle: 'Save Medical QR Code', UTI: 'public.png' });
          } else {
            Alert.alert("Error", "Sharing is not available on this device.");
          }
        } catch (error) { Alert.alert("Error", "Could not generate shareable image."); }
      });
    };
    ...
    const handleBloodChange = (text) => {
      ...
      const validBloodTypes = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];
      const upperText = text.toUpperCase();
      if (validBloodTypes.some(type => type.startsWith(upperText))) { setFormData({ ...formData, blood: upperText }); } 
      else { Alert.alert("Invalid Blood Type", "e.g., A+, O-"); setFormData({ ...formData, blood: '' }); }
    };
    ...
    const handleGenerateQR = () => {
      ...
      if (!formData.age) { Alert.alert("Missing Info", "Age is required."); return; }
      if (!formData.blood) { Alert.alert("Missing Info", "Blood Type is required."); return; }
      if (!formData.phone.trim()) { Alert.alert("Missing Info", "Personal Phone number is required."); return; } // <-- Added Phone validation
      if (formData.contacts.length === 0) { Alert.alert("Missing Info", "At least one Emergency Contact is required."); return; }

      const validBloodTypes = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'];
      const cleanBlood = formData.blood.trim().toUpperCase();

      if (!validBloodTypes.includes(cleanBlood)) { Alert.alert("Invalid Blood Type", "Please enter a valid type (e.g. O+)."); return; }
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.card}>
              ...
              <View style={{ marginTop: 15 }}>
                ...
                <TextInput style={styles.input} value={formData.phone} onChangeText={(t) => setFormData({ ...formData, phone: t })} placeholder="(123) 456-7890" keyboardType="phone-pad" />
              </View>
            </View>

            {/* <-- ADDED INSURANCE UPLOAD CARD --> */}
            <View style={styles.card}>
              <Text style={styles.label}>Health Insurance (Optional)</Text>
              
              {formData.insurance ? (
                <View style={styles.insuranceTicket}>
                  <Text style={styles.insProvider}>{formData.insurance.provider}</Text>
                  <Text style={styles.insText}>Policy: {formData.insurance.policyNumber}</Text>
                  <Text style={styles.insText}>Group: {formData.insurance.groupNumber}</Text>
                </View>
              ) : null}
              ...
              >
                ...
                <Text style={styles.ocrBtnText}>
                  {isScanningOCR ? "Extracting Data..." : "Upload Insurance Card"}
                </Text>
              </TouchableOpacity>
            </View>
  ...
  const styles = StyleSheet.create({
    ...
    // <-- ADDED OCR STYLES -->
    ocrBtn: { backgroundColor: '#4F46E5', padding: 16, borderRadius: 14, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 10 },
    ocrBtnText: { color: 'white', fontWeight: 'bold', fontSize: 16 },
    insuranceTicket: { backgroundColor: '#F0FDF4', padding: 15, borderRadius: 12, borderWidth: 1, borderColor: '#BBF7D0', marginBottom: 15 },
    insProvider: { fontWeight: '900', color: '#166534', fontSize: 16, marginBottom: 4 },
    insText: { color: '#15803D', fontWeight: '600' }
  });